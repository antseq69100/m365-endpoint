...
# filtrés par appartenance à une OU spécifique
#
...
# - Recherche native AD via SearchBase + SearchScope Subtree
# - Choix interactif du mode d’affichage console
# - Choix interactif du filtre sur les comptes actifs
...
Write-Host ""
Write-Host "- Les sous-OU de cette OU seront incluses automatiquement via SearchScope Subtree" -ForegroundColor Gray
Write-Host ""
...
Write-Host ""
Write-Host "Choix du filtre sur les comptes :" -ForegroundColor Yellow
Write-Host "1 - Tous les comptes (actifs et inactifs)" -ForegroundColor Gray
Write-Host "2 - Uniquement les comptes actifs" -ForegroundColor Gray
$ActiveFilterChoice = Read-Choice -PromptMessage "Entre 1 ou 2" -AllowedValues @("1","2")
...
try
{
    $GroupMemberSamAccountNames = Get-ADGroupMember -Identity $GroupName -Recursive -ErrorAction Stop |
...
############################################################
# LECTURE DES UTILISATEURS DE L'OU ET SOUS-OU
############################################################

Show-Section -Title "Lecture des utilisateurs dans l'OU"

try
{
    $UsersInOU = Get-ADUser `
        -SearchBase $SearchOU `
        -SearchScope Subtree `
        -Filter * `
...
{
    Write-Host "Erreur lors de la lecture de l'OU." -ForegroundColor Red
    Write-Host "Vérifie le DN saisi et l'existence de l'OU." -ForegroundColor Yellow
    Write-Host $_.Exception.Message -ForegroundColor DarkYellow
    Pause
    return
}
...
############################################################
# FILTRE SUR LES COMPTES ACTIFS
############################################################

if ($ActiveFilterChoice -eq "2")
{
...
{
    Write-Host "Aucun utilisateur trouvé dans le groupe et l'OU spécifiée avec le filtre choisi." -ForegroundColor Red
}
else
{
    Write-Host ("Nombre d'utilisateurs trouvés : {0}" -f $FilteredUsers.Count) -ForegroundColor Green

    if ($ActiveFilterChoice -eq "1")
    {
        Write-Host "Filtre appliqué : tous les comptes" -ForegroundColor DarkGray
    }
    else
    {
        Write-Host "Filtre appliqué : comptes actifs uniquement" -ForegroundColor DarkGray
    }
